...
gabierto_smooth = gaussian_filter1d(gabierto_normalized, sigma=2)
gcascara_smooth = gaussian_filter1d(gcascara_normalized, sigma=2)
gexpuesto_smooth = gaussian_filter1d(gexpuesto_normalized, sigma=2)

# Crear una nueva figura para las firmas normalizadas
fig, ax = plt.subplots(figsize=(12, 8))

# Graficar las firmas espectrales normalizadas y suavizadas
ax.plot(filtered_wavelength, gabierto_smooth, label='Abierto (Suavizado)', color='red')
ax.plot(filtered_wavelength, gcascara_smooth, label='Cáscara (Suavizado)', color='green')
ax.plot(filtered_wavelength, gexpuesto_smooth, label='Expuesto (Suavizado)', color='blue')

# Configuración del gráfico
ax.set_xlabel('Wavelength (nm)')
ax.set_ylabel('Normalized Reflectance')
ax.set_title('Spectral Signatures (Min-Max Normalized) - Abierto, Cáscara, Expuesto (Suavizado)')
ax.legend()

...
# Aplicar un filtro gaussiano para suavizar las firmas de los materiales
espuma_media_smooth = gaussian_filter1d(espuma_media, sigma=2)
latex_media_smooth = gaussian_filter1d(latex_media, sigma=2)
silicona_media_smooth = gaussian_filter1d(silicona_media, sigma=2)
aluminio_media_smooth = gaussian_filter1d(aluminio_media, sigma=2)

# Normalizar todas las firmas de materiales utilizando MinMaxScaler
scaler = MinMaxScaler()
espuma_media_smooth = scaler.fit_transform(espuma_media_smooth.reshape(-1, 1)).squeeze()
latex_media_smooth = scaler.fit_transform(latex_media_smooth.reshape(-1, 1)).squeeze()
silicona_media_smooth = scaler.fit_transform(silicona_media_smooth.reshape(-1, 1)).squeeze()
aluminio_media_smooth = scaler.fit_transform(aluminio_media_smooth.reshape(-1, 1)).squeeze()
...
# Graficar las firmas espectrales medias para los materiales (suavizadas)
ax.plot(filtered_wavelength, espuma_media_smooth, label='Espuma')
ax.plot(filtered_wavelength, latex_media_smooth, label='Latex')
ax.plot(filtered_wavelength, silicona_media_smooth, label='Silicona')
ax.plot(filtered_wavelength, aluminio_media_smooth, label='Aluminio')

# Graficar las firmas combinadas para grano abierto y cerrado en color negro (sin suavizar)
